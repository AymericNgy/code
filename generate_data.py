...
def generate_txt_random(length, num_files, output_folder="./data_LZ77/text", repetition_probability=0.8):

    for i in range(num_files):
        random_bytes, _ = generate_random_ascii_bytes(length, repetition_probability)
        filename = os.path.join(output_folder, f'text_{i + 1}.txt')

        with open(filename, 'wb') as file:
            file.write(random_bytes)


def generate_compress_file_LZ77(output_folder="./data_LZ77/compress", input_folder="./data_LZ77/text"):
    ...
    for filename in os.listdir(input_folder):
        ...
        compress_path = os.path.join(output_folder, filename)

        compressor.compress(file_path, compress_path)
